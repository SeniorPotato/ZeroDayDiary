# Queue

## TRIAGED
- **Kerala student cyber safety protocol**
  - Category: governance
  - Supporting tags: surveillance, public-sector, education
  - Type: event / policy signal
  - Confidence: medium
  - Next action: upgrade from watchlist framing if primary government source is located

- **Enterprise AI extension / interface-layer risk**
  - Category: ai-risk
  - Supporting tags: security, infrastructure, model-misuse
  - Type: analysis + event pattern
  - Confidence: medium
  - Next action: collect stronger primary evidence or official advisory material

- **Privacy guardrails as AI deployment resilience**
  - Category: privacy
  - Supporting tags: ai-risk, governance, policy-shift
  - Type: analysis / framing piece
  - Confidence: medium
  - Next action: connect to primary regulator or policy documents where possible

## DRAFTING
- FTC COPPA age-verification policy statement → strong privacy / children / platform-governance candidate
- ENISA Threat Landscape 2025 summary → strong security / threat-trends candidate
- ENISA to operate the EU Cybersecurity Reserve with EUR 36 million → strong infrastructure / policy candidate
- EDPB work programme 2026-2027 compliance templates initiative → strong governance / privacy candidate
- Daily watchlist / roundup cadence post template

## REVIEW
- CISA guidance on ongoing global exploitation of Cisco SD-WAN systems → drafted and published 2026-03-10
- EDPB / GPA statement on AI-generated imagery and privacy harms → drafted and published 2026-03-10
- EDPB right to erasure enforcement report → drafted and published 2026-03-10
- EDPB / EDPS Digital Omnibus joint opinion → drafted and published 2026-03-10

## HOLD
- CISA KEV individual CVE additions → useful for roundups unless tied to a stronger operational story
- ENISA cybersecurity investment piece → useful for broader analysis, weaker as a standalone event post
- ENISA NIS2 implementation guidance → hold unless combined into a compliance / implementation roundup
- FTC workshop / inquiry notices without direct security or privacy-impact signal → hold unless they produce stronger downstream action
- ISO standards / insights items without clear cyber, privacy, or governance event signal → hold or reject depending on downstream relevance
- any item discovered only through weak aggregation without a stronger source path
- items better suited to later pattern analysis rather than immediate publication

## REJECTED
- general ISO items unrelated to cybersecurity, privacy, AI governance, or digital regulation
- FTC competition / antitrust items without meaningful security, privacy, or platform-governance relevance
- pure opinion pieces without event signal
- vendor promotion framed as incident reporting

## PUBLISHED
- Welcome to ZeroDayDiary
- Publishing model and guardrails
- Global breach tracking needs better memory
- Privacy changes are often operational, not theoretical
- AI risk reporting should focus on deployment reality
- Surveillance is expanding through ordinary systems
- Global risk journals need taxonomy, not chaos
- Last 48 hours watchlist: three signals worth tracking
- Why the next wave of AI security stories will be about interfaces
- Student cyber safety is becoming a global policy issue
- Privacy guardrails are becoming an AI competitiveness question
